...
if [[ $# != 2 ]];then
	echo "Usage: $0 [storage] [parameters] [storage]"
	exit 1
fi
...
case $PARAMETER in
	...
	;;	
	size_nlsas)
		SIZE_SASNL=$(cat $FILE | grep "Array F4th_SASNL_001 (RAID 5)" | awk -F"\\\) \\\(" '{print $2}' | awk -F")" '{print $1}' | tr -d [,.] | awk -F" " '{print $1}')
		echo $SIZE_SASNL
	;;
	size_sas)
		SIZE_SASNL=$(cat $FILE | grep "Array F4th_SAS_001 (RAID 5)" | awk -F"\\\) \\\(" '{print $2}' | awk -F")" '{print $1}'| tr -d [,.] | awk -F" " '{print $1}')
		echo $SIZE_SASNL
	;;
	size_free_nlsas)
		FREE_NLSAS=$(cat $FILE | grep "F4th_SASNL_001" | grep "Serial Attached SCSI" | awk '{print $7}' | tr -d [,.] | awk -F" " '{print $1}')
		echo $FREE_NLSAS
	;;
	size_free_sas)
		FREE_SAS=$(cat $FILE | grep " F4th_SAS_001" | grep "Serial Attached SCSI" | awk '{print $7}' | tr -d [,.] | awk -F" " '{print $1}')
		echo $FREE_SAS
	;;
	size_used_nlsas)
		USED_NLSAS=$(cat $FILE | grep "F4th_SASNL_001" | grep "Serial Attached SCSI" | awk '{print $5}' | tr -d [,.] | awk -F" " '{print $1}')
		#USED_NLSAS=$(expr $USED_NLSAS / 1024)
		echo $USED_NLSAS
	;;
	size_used_sas)
		USED_SAS=$(cat $FILE | grep "F4th_SAS_001" | grep "Serial Attached SCSI" | awk '{print $5}')
		echo $USED_SAS
	;;
esac
