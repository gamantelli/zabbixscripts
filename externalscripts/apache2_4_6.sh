...
if [[ $# ==  1 ]];then
    #Agent Mode
    VAR=$(wget --no-proxy --quiet -O - http://127.0.0.1/server-status?auto --no-check-certificate)
    CASE_VALUE=$1
elif [[ $# == 2 ]];then
    #External Script Mode
    VAR=$(wget --quiet -O - http://$1/server-status?auto --no-check-certificate)
    CASE_VALUE=$2
else
    #No Parameter
    usage
